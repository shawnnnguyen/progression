...
export function LabelsField({
  ...
  const selectedIds = new Set(ticket.labels.map((label) => label.id));
  ...
  return (
    <div className={compact ? undefined : "flex flex-col gap-1"}>
      {!compact && <span className={FIELD_LABEL_CLASSNAME}>Labels</span>}
      <div className="flex flex-wrap items-center gap-1.5">
        {ticket.labels.map((label) => (
          <LabelTag key={label.id} label={label} onRemove={disabled ? undefined : () => toggle(label.id, false)} />
        ))}
        <DropdownMenu>
          <DropdownMenuTrigger
            disabled={disabled}
            className={fieldTriggerClassName(compact, "border-dashed text-[var(--color-neutral-500)]")}
          >
            + Add
          </DropdownMenuTrigger>
